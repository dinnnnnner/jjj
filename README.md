...
CAN SENT 解码器位于 `src/protocol/can_data.rs`。ID 1 按小端 `i16` 解出 T2 angle、T2 torque、S angle；ID 2 解出 T1 angle、T1 torque。即使填充到 64 字节，这两种格式也只解码一次。其他非 TX 且非 ID 3 的足长帧通过 `decode_sent_values` 解析为 `f32`：
...
- `sent_filter_enabled`：CAN SENT 入口处的移动平均滤波。`SentMovingAverage` 在 `src/signal/mod.rs`，对整数和浮点三种格式都生效；按设备、通道和信号维护窗口，只更新当前帧包含的信号。角度信号使用圆周平均，扭矩信号使用算术平均。
...
修复后的采集端与 UI 使用 feed 协议 v3，必须一起更新和重启。实时曲线按设备、数据来源和传感器隔离，设备下拉框控制对应视图使用的设备。自动视图识别仅执行一次，后续样本不会重置用户窗口。CAN 回放和导出每次查询一个设备，留空时选择该时段第一个设备，可从已发现设备列表切换。

CAN 在 DLL 回调入口记录主机时间，以每通道首个有效硬件微秒时间戳建立映射，后续样本保留硬件时间差；时间戳为零时使用回调时间，并在下一个有效时间戳重新校准；硬件时间倒退时也重新校准。输出时间不小于上一帧，主机时钟倒退时同样保持此约束。同帧所有信号共用时间。绝对时间仍包含首次回调的延迟，不保证跨设备时钟同步；每通道回调需要按时间顺序到达。
...
当前恢复机制使用有界内存缓存（写入队列 50,000 条及当前批次），不是磁盘日志。长时间中断超过缓存容量会产生已计数的总线丢弃，进程退出也会失去未写入数据。重试为至少一次语义：提交成功但响应丢失时可能出现重复记录。UI 每秒接收活动告警快照以校正漏掉的触发/恢复。增量和快照在告警状态锁内进入同一个专用队列，由单一任务依次转发；连接时请求的快照也遵循此顺序，避免快照抢先造成历史漏计。快照和增量携带采集进程标识 `epoch` 与递增序号 `revision`，旧增量不会覆盖新快照，较旧快照会保留之后的增量。快照校正当前状态，不补齐丢失的历史事件。
