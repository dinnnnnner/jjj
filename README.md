...
CAN 在 DLL 回调入口记录主机时间，以每通道首个有效硬件微秒时间戳建立映射，后续样本保留硬件时间差；时间戳为零时使用回调时间，倒退时视为时钟重置并重新校准。同帧所有信号共用时间。绝对时间仍包含首次回调的延迟，不保证跨设备时钟同步；每通道回调需要按时间顺序到达。
...
当前恢复机制使用有界内存缓存（写入队列 50,000 条及当前批次），不是磁盘日志。长时间中断超过缓存容量会产生已计数的总线丢弃，进程退出也会失去未写入数据。重试为至少一次语义：提交成功但响应丢失时可能出现重复记录。UI 每秒接收活动告警快照以校正漏掉的触发/恢复。快照和增量携带采集进程标识 `epoch` 与递增序号 `revision`，旧增量不会覆盖新快照，较旧快照会保留之后的增量。快照校正当前状态，不补齐丢失的历史事件。
