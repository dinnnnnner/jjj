...
const DEFAULT_EXPORT_PATH: &str =
    "D:\\xxdd\\tas\\xd\\exports\\can_export_04-17_09-48-45_04-16_16-43-34_04-17_09-48-34.txt";
...
fn parse_replay_file(path: &str) -> anyhow::Result<Vec<ReplayRow>> {
    let text =
        std::fs::read_to_string(path).with_context(|| format!("read export failed: {path}"))?;
