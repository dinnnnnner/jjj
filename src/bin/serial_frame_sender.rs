...
fn run_legacy_sender(cfg: &Config, port: &mut dyn serialport::SerialPort) -> anyhow::Result<()> {
    ...
    for tick in 0..total_ticks {
        ...
        ack_frames =
            ack_frames.saturating_add(drain_ack(port, &codec, &mut ack_buf, &mut ack_chunk)?);

        thread::sleep(Duration::from_millis(cfg.interval_ms));
    }

    ack_frames = ack_frames.saturating_add(drain_ack(port, &codec, &mut ack_buf, &mut ack_chunk)?);
