...
const DEFAULT_CAN_EXPORT_PATH: &str = "C:\\Users\\admin\\Desktop\\xd\\target\\debug\\exports\\can_export_04-22_13-18-27_04-22_13-13-25_04-22_13-18-25.txt";
...
fn sent_channel_value(pair_id: usize, t_sec: f64, phase_shift_deg: f64, amplitude: f64) -> u16 {
    ...
    let base =
        ((2.0 * std::f64::consts::PI * t_sec / 3.0) + pair_phase + phase_shift_deg.to_radians())
            .sin()
            * amplitude;
...
fn demo_axis_value(axis: usize, t_sec: f64) -> i16 {
    ...
    let ripple = ((2.0 * std::f64::consts::PI * 5.8 * t_sec) + phase * 0.6).sin() * 220.0;
    let fine = ((2.0 * std::f64::consts::PI * 12.0 * t_sec) + phase * 1.3).sin() * 90.0;
    let bias = match axis {
        0 => 120.0,
        1 => -80.0,
        _ => 45.0,
    };
    (base + ripple + fine + bias).round().clamp(-8192.0, 8191.0) as i16
}
    ...
    for tick in 0..total_ticks {
        ...
        for sensor_id in 0..config.sensor_count {
            ...
            write_frame_all(port, &out).with_context(|| {
                format!(
                    "serial write failed on {} for req_id={}",
                    config.port, frame.request_id
                )
            })?;
...
async fn run_sent_sender(
    ...
    let frame_us = config
        .format
        .frame_us()
        .context("SENT mode requires fixed frame length")?;
    let start = Instant::now();
    let deadline = start + Duration::from_secs(config.duration_secs);
    let mut ticker = time::interval(Duration::from_micros(frame_us));
    ticker.set_missed_tick_behavior(MissedTickBehavior::Delay);
        ...
        for pair_id in 0..config.sensor_count {
            ...
            write_frame_all(port, &raw).with_context(|| {
                format!(
                    ...
                    config.port, sent_frames, pair_id
                )
            })?;
            sent_frames = sent_frames.saturating_add(1);
            ...
            let result = (|| -> anyhow::Result<()> {
                ...
                rt.block_on(async move {
                    ...
                    for (idx, row) in rows.into_iter().enumerate() {
                        ...
                        transport.transmit(frame).await.with_context(|| {
                            format!("transmit CAN export row failed at index {idx}")
                        })?;
