...
type SetMappingVerbose = unsafe extern "system" fn(
    *const c_char,
    i32,
    i32,
    *const c_char,
...
impl TsMasterApi {
    unsafe fn load(dll_path: &Path) -> Result<Self, CanTransportError> {
        let lib =
            unsafe { Library::new(dll_path) }.map_err(|err| io::Error::other(err.to_string()))?;
...
impl CanTransport {
    ...
    pub async fn connect(&mut self) -> Result<(), CanTransportError> {
        ...
        check("set_libtsmaster_location", unsafe {
            (api.set_libtsmaster_location)(runtime_dir_c.as_ptr())
        })?;
        check("initialize_lib_tsmaster", unsafe {
            (api.initialize_lib_tsmaster)(app_name.as_ptr())
        })?;
        ...
        let init_result = (|| -> Result<(), CanTransportError> {
            ...
            let max_channel = channels.iter().map(|ch| ch.index).max().unwrap_or(0);
            check("tsapp_set_can_channel_count", unsafe {
                (api.tsapp_set_can_channel_count)(i32::from(max_channel) + 1)
            })?;
            for ch in &channels {
                check("tsapp_set_mapping_verbose", unsafe {
                ...
                check("tsapp_configure_baudrate_canfd", unsafe {
                    (api.tsapp_configure_baudrate_canfd)(
                        i32::from(ch.index),
                        ch.arbitration_baud_kbps as f32,
                        ch.data_baud_kbps as f32,
                        CANFD_TYPE_ISO,
                        CANFD_MODE_NORMAL,
                        1,
                    )
            ...
            check("tsapp_register_event_can", unsafe {
                (api.tsapp_register_event_can)(&mut self.event_obj, on_can_event)
            })?;
            if let Some(register_canfd) = api.tsapp_register_event_canfd {
                check("tsapp_register_event_canfd", unsafe {
                    register_canfd(&mut self.event_obj, on_canfd_event)
                })?;
            }
            Ok(())
        })();
    ...
    pub async fn transmit(&self, frame: CanTxFrame) -> Result<(), CanTransportError> {
        ...
        if frame.dlc > 8 {
            ...
            check("tsapp_transmit_canfd_async", unsafe {
                transmit_canfd(&raw)
            })?;
        } else {
            let raw = frame.to_raw();
            check("tsapp_transmit_can_async", unsafe {
                (api.tsapp_transmit_can_async)(&raw)
            })?;
        }
        Ok(())
    }
